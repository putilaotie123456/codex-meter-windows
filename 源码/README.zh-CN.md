...
```powershell
.\源码\install.ps1 -Launch -StartWithWindows
# 或在解压目录：
...
```powershell
.\源码\dist\CodexMeter.Tests.exe --live
```

只验证本机近 7 天日志统计和增量缓存：

```powershell
.\源码\dist\CodexMeter.Tests.exe --weekly-live
```

进行只读资源耐久采样（默认 10 分钟，结果写入 `源码\qa`）：

```powershell
.\源码\tools\measure-resource-soak.ps1 -Minutes 10
```
